...
where  the numerator $P(x)$ and denominator $Q(x)$ are polynomials in $\Int[x]$, the denominator is nonzero, and fractions are considered equal up to cancelling common factors. Addition and multiplication are defined as usual, and a fraction is 0 if and only if its numerator is 0; two fractions are equal if and only if their difference is 0. We write $\Int(x)$ for the rational functions; this is a field. Let $A$ be a proper series. Then $A^+ \times (1-A) = A$, hence $A^+$ can be seen as a fraction of polynomials of $A$; by induction, all proper rational power series  are fractions of polynomials, which yields the embedding. 
