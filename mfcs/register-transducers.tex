...
To approach the equivalence problem for graph-to-graph transductions, we use register transducers that input trees. The idea is that graphs are given by tree decompositions. This leads to yet another notion of trees, as compared to the three notions described in Example~\ref{ex:trees}, namely trees over a ranked alphabet (which are roughly the same thing as terms without variables).
...
\mypic{4}
We use standard tree terminology, such as descendant, ancestor or parent. From now on, when talking about trees, we mean the ranked trees described above, and not any of the other trees discussed in Example~\ref{ex:trees}.
