...
\begin{definition}[Register transducer] \label{def:register-transducer} Let $\aalg$ be an algebra (in the sense of universal algebra, i.e.~a set with some operations on it). The syntax of a  \emph{nondeterministic tree-to-$\aalg$ register transducer} consists of:
    \begin{enumerate}
...
\begin{corollary}
The equivalence and functionality  problems for graph-to-graph \mso transductions of bounded treewidth  reduce to the functionality problem for nondeterministic \treetotreewidth{k} register transducers. 
\end{corollary}
